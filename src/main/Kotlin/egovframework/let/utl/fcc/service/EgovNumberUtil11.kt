...
object EgovNumberUtil {
    private val LOGGER: Logger = LoggerFactory.getLogger(EgovNumberUtil::class.java)
     ...
     * @see
     */
    fun getNumSearchCheck(sourceInt: Int, searchInt: Int): Boolean {
        val sourceStr = sourceInt.toString()
        val searchStr = searchInt.toString()
     ...
     * @see
     */
    fun getNumToStrCnvr(srcNumber: Int): String {
        var rtnStr: String = null

        rtnStr = srcNumber.toString()
     ...
     * @see
     */
    fun getNumToDateCnvr(srcNumber: Int): String {
        var pattern: String? = null
        var cnvrStr: String = null

        val srcStr = srcNumber.toString()

        // Date 형태인 8자리 및 14자리만 정상처리 
        require(!(srcStr.length != 8 && srcStr.length != 14)) { "Invalid Number: " + srcStr + " Length=" + srcStr.trim { it <= ' ' }.length }
        ...
        val dateFormatter = SimpleDateFormat(pattern, Locale.KOREA)

        var cnvrDate: Date? = null
     ...
     * @see
     */
    fun getNumberValidCheck(checkStr: String): Boolean {
        var i: Int

        //String sourceStr = String.valueOf(sourceInt);
        val checkStrLt = checkStr.length

        i = 0
        while (i < checkStrLt) {
            // 아스키코드값( '0'-> 48, '9' -> 57)
            if (checkStr.get(i).code > 47 && checkStr.get(i).code < 58) {
                i++
                continue
            } else {
                return false
     ...
     * @see
     */
    fun getNumberCnvr(srcNumber: Int, cnvrSrcNumber: Int, cnvrTrgtNumber: Int): Int {
        // 입력받은 숫자를 문자열로 변환

        var source = srcNumber.toString()
        val subject = cnvrSrcNumber.toString()
        val `object` = cnvrTrgtNumber.toString()
     ...
     * @see
     */
    fun checkRlnoInteger(srcNumber: Double): Int {
        ...
        var cnvrString: String? = null

        if (srcNumber < 0) {
            return -1
        } else {
            cnvrString = srcNumber.toString()
